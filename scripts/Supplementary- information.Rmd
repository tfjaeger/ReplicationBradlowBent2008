...
Performance during the earliest exposure blocks was higher in Experiment 1b, compared to 1a, for the multi-talker condition, and to some extent for the control condition. What these two these two exposure conditions have in common is that they involve five exposure talkers, rather than one (for the same total number of stimuli), and thus more heterogenous stimuli. Other than this difference, performance seems pretty much identical across exposure conditions. 

It is possible that differences in performanec during exposure reflect difference in the audio equipment between Experiments 1a and 1b. In particular, the proportion of participants who used headsets remained identical betwween Experiments 1a and 1b only for the talker-specific condition. The proportion of participants who used headsets increased most from Experiment 1a to 1b for the control and multi-talker conditions---i.e. precisely those conditions for which performance in Experiment 1b seems higher (during the earliest exposure trials), compared to Experiment 1a. Alternatively (or additionally), it is possible that the differences between the two experiments reflect changes in the population that we are recruiting from (about three years passed between recruitment for Experiment 1a and 1b).

**The analyses of performance during the test phase therefore include predictor controlling for individual differences in performance during exposure**. Below, we describe how this control predictor was obtained.^[Additional analyses previously conducted for Experiment 1a (which had a more asymmetric distribution of audio equipment across exposure conditions, see above) found that results did not change if audio equipment was included as a control predictor in the analyses.]
...
## Comparison of exposure phase in Experiments 1a and 1b [MORE TEXT HERE. EXPLAIN THE VARIOUS WAYS TO ANALYZE EXPOSURE DATA]
...
## Linear model of changes in performance during exposure ignoring experiment
...
## Monotonic model of changes in performance during exposure ignoring experiment

To ascertain that the linearity assumption in the linear model does not introduce undue bias to the estimation of the effects at the beginning of the experiment, we also fit a model that did only assume monotonicity of the changes across qunitiles, rather than the stronger qssumption of linearity (in log-odds). 
...
## Comparing different ways to correct for differences in exposure performance across experiments (TEXT NEEDS TO BE UPDATED)
...
To capture these differences we calculate the participant-specific *a priori* performance in the first trial bin of exposure. These are by-participant random intercepts for the proportion of keywords correctly transcribed during the first bin of exposure, corrected for the exposure condition (fixed effect), by-item and by-current talker random intercepts. We estimate two such corrections---one by using data from only Experiment 1a or only Experiment 1b, the other while using using data from both experiments combined (without a main effect of experiment, because we are interested in catching *a priori* differences in participant-specific performance, rather than attributing it to the version of the experiment).

These plots compare the two different approaches to correction for *a priori* performance of participants. In the remainder of this document, we use the second approach, the one based on fitting the correction model to **all** participants (i.e., both Experiments 1a and 1b, without an effect of Experiment). The plots shown here provide an intuition how this will affect the analyses of the test data in Experiments 1a and 1b, and how this differs across the four exposure conditions. The black shapes show the mean across participants in that condition and experiment.

### Comparing the individual differences for the linear model depending on whether effects are estimated while controlling for experiment
...
d.exp1ab.test %>%
  ...
  { . ->> temp } %>%
ggplot(aes(x = individual_training_performance.linear.wExperiment, y = individual_training_performance.linear, shape = Experiment, color = Experiment)) +
  geom_abline(slope = 1) +
  geom_point(alpha = .5) +
  ...
  scale_x_continuous("by-participant adjustment based on Experiments 1a and 1b") +
  scale_y_continuous("by-participant adjustment based on Experiments 1a *or* 1b only") +
...
### Comparing the individual differences for the linear and monotonic model depending (ignoring experiment)

Finally, there is no evidence that the linearity assumption causes any bias in the estimation of individual differences, compared to the model that merely assumes monotonicity. The two models provide *very similar* estimates of participants' performance during the first exposure block. In the remainder of the analyses, we thus used the linear model because it is more common and readers are more familiar with it. Given the high similarity in the estimates, we never investigated whether individual differences based on the monotonic model would change any of the results for the test data.
...
d.exp1ab.test %>%
  ...
  dplyr::summarise_at(vars(starts_with("individual_training_performance")), mean) %>%
  { . ->> temp } %>%
ggplot(aes(x = individual_training_performance.monotonic, y = individual_training_performance.linear, shape = Experiment, color = Experiment)) +
  geom_abline(slope = 1) +
  geom_point(alpha = .5) +
  geom_point(data = temp %>%
         group_by(Experiment, Condition2) %>%
...
m.exp1a.test.fromTraining <- brm(IsCorrect ~ 
                         ...
                         prior = my.priors, file = "../models/exp1a.Test.fromIndividualTrainingDifferences")
summary(m.exp1a.test.fromTraining)
```

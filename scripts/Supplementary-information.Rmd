...
In order to estimate the power for Questions 1-3 (and talker-specific adaptation, for the sake of comparison), each simulation fit both a model with sliding difference coding for exposure condition, and a model with treatment-coding for condition (see code in R markdown). This means that the three simulations presented below required 4 * 1000 * 2 = 8000 GLMM fits. On a 2013 PowerMac, ten simulated data sets and analyses for the BB08 data took about 2-3 minutes to complete if run on 10 cores. All three simulations together took about 2 days to complete.

When models did not converge, we report so below, rather than to refit the model with reduced random effect structure.
...
get_convergence = function(d) {
  with(d, 
       rbind(
         round(prop.table(table(!ConvergenceFailure.treat)), 2) * 100,
         round(prop.table(table(!ConvergenceFailure.treat)), 2) * 100,
         round(prop.table(table(!ConvergenceFailure.treat)), 2) * 100,
         round(prop.table(table(!ConvergenceFailure.diff)), 2) * 100,
         round(prop.table(table(!ConvergenceFailure.diff)), 2) * 100
       )[,2])
}

get_significant_wo_convergence_failure = function(d) {
  with(d, 
       rbind(
...
get_significance_and_convergence = function(d) {
  paste0(
    get_significant_wo_convergence_failure(d),
    "% (",
    get_convergence(d),
    "%)"
  )
...
kable(
  matrix(cbind(
    get_significance_and_convergence(d.sim.small),
    get_significance_and_convergence(d.sim),
    get_significance_and_convergence(d.sim.2var),
    get_significance_and_convergence(d.sim.half)
  ), nrow = 5, ncol = 4, 
  dimnames = list(

...
labels.exposure = c("Control", "Single-talker", "Multi-talker", "Talker-specific")
...
defineContrasts = function(dataset) {
  ...
  dataset %<>%
    mutate(
      ...
      # that the ordering of Condition (used in e.g., empirical plots) is the reverse
      # of the order of Cond.treat and Cond.diff
      Cond.treat = factor(Condition, levels = rev(levels.exposure)),
      ...
      # For the sake of continuity (previously run models), reverse factor order here
      Cond.diff = factor(Condition, levels = rev(levels.exposure)),
...
plot_3effects = function(
  ...
  # If BFs are meant to be plotted, make a table of hypotheses 
  if (addBFs) {
    group = if (by == "TestTalker") "TestTalkerID" else "NULL"
    scope = if (by == "TestTalker") "coef" else "standard"
    h = rbind(
      hypothesis(model, 
                 "Cond.diffTS.vs.MT + Cond.diffMT.vs.ST + Cond.diffST.vs.CNTL > 0", 
                 class = "b", scope = scope, group = group)$hypothesis %>%
        mutate(condition = "Cond.TS.vs.CNTL"),
      hypothesis(model, "Cond.diffMT.vs.ST + Cond.diffST.vs.CNTL > 0", 
                 class = "b", scope = scope, group = group)$hypothesis %>%
        mutate(condition = "Cond.MT.vs.CNTL"),
      hypothesis(model, "Cond.diffST.vs.CNTL > 0", 
                 class = "b", scope = scope, group = group)$hypothesis %>%
        mutate(condition = "Cond.ST.vs.CNTL"),
      hypothesis(model, "Cond.diffMT.vs.ST > 0", 
                 class = "b", scope = scope, group = group)$hypothesis %>%
        mutate(condition = "Cond.MT.vs.ST")) %>%
      { if ("Group" %in% names(.)) rename(., TestTalker = Group) } %>%
      mutate(Evid.Ratio = ifelse(is.infinite(Evid.Ratio), 
                                 paste0("BF>", 
                                        nrow(hypothesis(
                                          model, 
                                          "Cond.diffMT.vs.ST > 0", 
                                          class = "b")$samples)),
                                 paste0("BF=", round(Evid.Ratio, 1))))
  }
  ...
  p = p + 
    ...
    { if (addBFs) geom_text(
      data = h,
      aes(label = Evid.Ratio, y = condition),
      x = 1.3, size = 2.75, hjust = 1, inherit.aes = FALSE,
...
plot_predicted_outcome = function(
  ...
  d = data %>%
    group_by(WorkerID, TestTalkerID, TrainingTalkerID, TrainingUnderTestTalkerID, Condition, Cond.diff, !! sym(offset.term), Sentence) %>%
  ...
  m = d %>%
    ...
    group_by(!!! syms(groups), Condition, .draw) %>%
    summarise(.value = mean(.value)) %>%
    mean_hdci()
    
  myGplot.defaults("paper")
  p = m %>%
    ggplot(aes(x = Condition, color = Condition, fill = Condition, y = .value)) +
    # actual subject data
    geom_dotplot(
      binaxis = "y",stackdir = "center", width = .1, binwidth = .01, 
      data = d %>% 
        group_by(WorkerID, Condition, !!! syms(groups)) %>%
    ...
    { if (length(groups) > 0) facet_wrap(facets = vars(!!! syms(groups[1])), nrow = nrow) }  +
  ...
  ggsave(p, 
         ...
         height = .2 + (if (length(groups) > 0) nrow * 3.5 else 3.5), 
         width = .5 + (if (length(groups) > 0) (nlevels(m[[groups[1]]]) * 3.7) / nrow else 6))
...
## load data 
## ------------------------------ ##
d.full = readRDS("./data/replication_data_all-February-2019.RDS")

# Exclusions (undo, if you're interested in the reasons for rejection, coded in ReasonRejected)
# We're also checking for duplicate lines (there aren't any)
...
## ------------------------------ ##
d.full %<>% 
  rename(SentenceSoundFile = Filename) %>%
  mutate(
    Experiment = case_when(
      Exp2_subj == "yes" ~ "1a",
      ExpRep_subj == "yes" ~ "1b",
      Exp1_subj == "yes" ~ "Pilot only",
      Exp3_subj == "yes" ~ "OrderTest",
      T ~ NA_character_
    ),
    PilotParticipant = ifelse(Exp1_subj == "yes", T, F),
    Condition = factor(ifelse(Condition2 == "Single talker", "Single-talker", as.character(Condition2)), 
                       levels = levels.exposure)
  ) %>%
  # Repair exposure talker ID
  arrange(Experiment, Condition, TestTalkerID, TrainingTestSet, ListNum, WorkerID, PartOfExp, Trial) %>%
  # Get the unique order of exposure talkers (for this, we group by part of experiment)
  group_by(Experiment, Condition, TestTalkerID, TrainingTestSet, ListNum, WorkerID, PartOfExp) %>%
  mutate(TrainingTalkerID = ifelse(PartOfExp == "training", paste(unique(CurrentTalkerID), collapse = "__"), NA)) %>%
  # Now make sure that this information is shared at the worker ID level (so that it will
  # be available for the test trials)
  group_by(Experiment, Condition, TestTalkerID, TrainingTestSet, ListNum, WorkerID) %>%
  ...
  mutate(
    TrainingTalkerID = last(TrainingTalkerID), # last, because "training" is sorted after "test"
    ...
    TrainingUnderTestTalkerID = case_when(
      Condition == "Single-talker" ~ paste(TrainingTalkerID, TestTalkerID, sep = "__"),
      T ~ "0"
    ),
    FirstTrainingTalkerID = gsub("^([A-Z]+\\_[A-Z]+\\_[0-9]+)__.*$", "\\1", TrainingTalkerID)) %>%
  ungroup() %>%
  # Remove redundant variables (to simplify this already rather large data frame)
  dplyr::select(-c(
    RejectWorker, ReasonRejected,
    TestTalkerNameShort, TestTalkerName, CurrentTalkerIDShort,
    ExpRep_subj, Exp1_subj, Exp2_subj, Exp3_subj,
    PresentationBlock, Condition2, Condition.long,
    Answer.condition, Answer.errors, Answer.list_num, Answer.rsrb.protocol, Answer.speaker)) %>%
  # Turn into factors
  mutate_at(
    c("Sentence", "WorkerID", 
      "TestTalkerID", "TrainingTalkerID", "TrainingUnderTestTalkerID", "FirstTrainingTalkerID", "CurrentTalkerID",
      "PartOfExp", "TrainingTestSet", "SentenceID"
      ),
    as.factor
  ) %>%
  droplevels() %>%
  dplyr::select(Experiment, PilotParticipant, ListNum, TrainingTestSet, PartOfExp, Condition, 
         ListPosition, Trial, WorkerID, 
         CurrentTalkerID, TrainingTalkerID, FirstTrainingTalkerID, TrainingUnderTestTalkerID, TestTalkerID,
         SentenceID, Sentence, SentenceSoundFile, 
         Keywords, NumKeywords, Keywords_perSet, PropKeywordsCorrect, NumKeywordsCorrect,
         everything())
```
...
d.full %<>%
  mutate(Keyword = as.character(Keywords)) %>%
...
d.pilot <- d.full %>%
  filter(PilotParticipant == T) %>%
  distinct() %>%
  droplevels() %>%
  mutate(Experiment = "Pilot")

# Experiment that test effects of item order
d.order <- d.full %>%
  filter(Experiment == "OrderTest" & TrainingTestSet != "set12345,set2") %>%
  distinct() %>%
  droplevels()

d.order.training_includes_test <- d.full %>%
  filter(Experiment == "OrderTest" & TrainingTestSet == "set12345,set2") %>%
  distinct() %>%
  droplevels()
...
# README FIRST
This markdown document presents all analyses reported in the paper, and provides additional tables, visualization, and analyses. We have intentionally *not* set a random seed for our analyses. Thus, the specific numbers for the Bayesian models might differ from those reported in the paper. Over the course of our analysis, we conducted all analyses repeatedly and did not notice qualitative changes in the results.

**Note that the analyses in this markdown document might take several days to complete.** The reason we do not provide the completed models as part of the repository is the size of the files. The small Bayesian models are about 10-30MB. The main analyses, meta-analysis, and the models necessary for the replication analyses are between 300-600MB each. All models together require over 10 GB of space. **If you are interested in specific models and do not have the computing environment to refit them, please contact the authors.**  
...
This section provides tables summarizing the participant counts across combinations of different exposure conditions, exposure talkers, and test talkers.
...
d.pilot %>% 
  ...
  group_by(Condition, TestTalkerID, Experiment) %>% 
  distinct(WorkerID) %>%
  xtabs(~TestTalkerID + Condition + Experiment, .)
```

### Experiments 1a and 1b

Level at which both experiments were balanced (unique combinations of exposure condition and test talker):
```{r}
d.exp1ab %>% 
  filter(PartOfExp == "test") %>%
  group_by(Condition, TestTalkerID, Experiment) %>% 
...
```{r}
d.exp1a %>%
  dplyr::select(Condition, TestTalkerID, TrainingTalkerID, WorkerID) %>%
  distinct() %>%
  xtabs(~ TrainingTalkerID + TestTalkerID + Condition, data = .)
```
...
d.pilot %>% 
  group_by(Experiment, Condition, TrainingTestSet, WorkerID, PartOfExp) %>% 
  summarise(NumOfKeywords = length(WorkerID)) %>%
  group_by(Experiment, TrainingTestSet, Condition, PartOfExp) %>% 
...
We provide basic demographic information about our participants. No other demographic information was elicited.
...
d.exp1ab.training %>%
  group_by(WorkerID, Sentence) %>%
  distinct(.keep_all = TRUE) %>%
  group_by(Experiment, WorkerID, Condition, Quintile) %>%
	dplyr::summarise(PropKeywordsCorrect = mean(PropKeywordsCorrect)) %>%
	ggplot(aes(x = Quintile + 1, y = PropKeywordsCorrect, colour = Condition, fill = Condition, shape = Experiment)) +
...
p = plot(conditional_effects(m.brm.exp1ab.train.wExperiment, effects = c("Cond.diff:Experiment", "sQuintile:Experiment"),
                         # set to NULL to add uncertainty about all random effects
                         re_formula = NA), ask = F, plot = F)
p[[1]] +
  scale_x_discrete("Exposure condition") +
  scale_y_continuous("Predicted proportion correct") +
  theme_bw()
...
xhypotheses(
  ...
  labels = c(
    "TS < CNTL (1st Block)", "MT < CNTL (1st Block)", "ST < CNTL (1st Block)", "MT < ST (1st Block)",
...
Whatever the reason for these differences, it is possible that they confound the analyses of the test data. **The analyses of performance during the test phase therefore control for individual differences in performance during exposure**. Specifically, the analyses of the test data use an offset term that is based on the individual difference in performance at the onset of the exposure phase. Next, we describe how we estimated these individual differences.^[Additional analyses previously conducted for Experiment 1a (which had a more asymmetric distribution of audio equipment across exposure conditions, see above) found that results did not change if audio equipment was included as a control predictor in the analyses.]
...
p = plot(conditional_effects(m.brm.exp1ab.train, effects = c("Cond.diff", "sQuintile:Cond.diff"),
                         re_formula = NA), ask = F, plot = F)
p[[1]] + 
  scale_x_discrete("Exposure condition") + 
  scale_y_continuous("Predicted proportion correct") +
  theme_bw()
p[[2]] + 
  scale_x_discrete("Trial bin\n(standardized)") + 
  scale_y_continuous("Predicted proportion correct") + 
  scale_color_discrete("Exposure condition") +
  scale_fill_discrete("Exposure condition") +
  theme_bw() +
...
p = d.exp1ab.test %>%
  dplyr::select(Experiment, WorkerID, Condition, starts_with("individual_training_performance")) %>%
  distinct() %>%
  group_by(Experiment, WorkerID, Condition) %>%
  dplyr::summarise_at(vars(starts_with("individual_training_performance")), mean) %>%
  { . ->> temp } %>%
  ggplot(
    aes(
      x = individual_training_performance.linear, 
      y = individual_training_performance.linear.wExperiment, 
      shape = Experiment, 
      color = Experiment)) +
  geom_abline(slope = 1) +
  geom_point(alpha = .5) +
...
```{r, fig.width=5}
myGplot.defaults(type = "paper")
p = plot(conditional_effects(m.brm.exp1ab.train.monotonic, 
                         # set to NULL to add uncertainty about all random effects
                         re_formula = NA), ask = F)
...
p = d.exp1ab.test %>%
  ...
  ggplot(
    aes(
      x = individual_training_performance.linear, 
      y = individual_training_performance.monotonic, 
      shape = Experiment, 
      color = Experiment)) +
  geom_abline(slope = 1) +
  geom_point(alpha = .5) +
...
```
The effect of the individual differences predictor can also be seen in the reduction of the variability in the by-participant random intercepts during test in the model that controlled for individual differences in performance---here shown for the combined data from all exposure conditions of both experiments (SD = .60):
...
d.exp1ab.test %>%
  filter(Condition == "Single-talker") %>%
  dplyr::select(individual_training_performance.linear) %>%
  summarise_at(vars(starts_with("individual_training_performance")), function(x) 2 * sd(x)) %>%
  as.numeric() %>%
...
d.exp1ab.test %>%
  filter(Condition == "Talker-specific") %>%
  dplyr::select(individual_training_performance.linear) %>%
  summarise_at(vars(starts_with("individual_training_performance")), function(x) 2 * sd(x)) %>%
  as.numeric() %>%
...
All models were fit with the settings described in the main text. We note that substantially fewer posterior samples would have been sufficient. A larger number of posterior samples was required for the replication test, but separate (numerically-coded) models were fit for that test anyway. The reason for using equally many warmup and posterior samples for the main analysis was to simplify the write-up.
...
plot_base = 
  ...
  dplyr::summarise(PropKeywordsCorrect = mean(PropKeywordsCorrect)) %>%
  ggplot(aes(x = Condition, y = PropKeywordsCorrect, colour = Condition, fill = Condition)) +
  ...
  scale_colour_manual("Exposure Condition", 
                      labels = labels.exposure,
                      values = setNames(colors.exposure, levels.exposure), guide=FALSE) +
  scale_fill_manual("Exposure Condition", 
                    labels = labels.exposure,
                    values = setNames(colors.exposure, levels.exposure), guide=FALSE) +
...
# Write to file
{
  sink(file = "./tables/table_hypotheses_Exp1a.tex")
  print(table_3effects(m.exp1a.test.diff, format = "latex"))
  sink()
}
```

### Talker-specific vs. multi-talker exposure

Due to space limitations, we do not discuss all results of theoretical relevance in the main text. Beyond talker-specific adaptation and Questions 1-3, the differences between the talker-specific and multi-talker conditions is also of interest. It indicates whether cross-talker generalization was essentially as good as the best possible performance. BB08 found no signficiant difference between their talker-specific and multi-talker condition. Here, we do not replicate that result. 
...
One possible explanation for this difference is that the exposure talkers in our experiments were *together* less similar to the test talkers, compared to the exposure and test talkers in BB08. If cross-talker generalization is at least in part based on similarity-based inferences (see General Discussion), this would explain why multi-talker exposure does not reach the same level of performances as talker-specific exposure in the present experiments.

An alternative, though not mutually exclusive, explanation is that the full benefit of multi-talker exposure only becomes apparent after longer exposure (recall that participants in BB08 were exposure to twice as many sentences over two exposure sessions distributed over two days). One reason to expect this to be the case is that participants in the multi-talker condition receive less input from each talker, so that additional input might help participants to form a more accurate model of the exposure talker's pronunciation. To the extent that an exposure talker's pronunciations are informative about the test talker's pronunciation, this is expected to facilitate performance during test.
...
### Questions 1-2 (via treatment-coding)
...
p = p.test.emplogit %+% (
  d.exp1b.test %>%
    ...
    group_by(WorkerID, Condition) %>%
    dplyr::summarise(PropKeywordsCorrect = emplog(mean(PropKeywordsCorrect), 51)))
ggsave(p, filename = "./figures/p.exp1b.emplogit.pdf", device = cairo_pdf, width = 6, height = 4, dpi=300)
```
...
# Write to file
{
  sink(file = "./tables/table_hypotheses_Exp1b.tex")
  print(table_3effects(m.exp1b.test.diff, format = "latex"))
  sink()
}
```
...
See Experiment 1a for relevance. While the support for a difference between the talker-specific and multi-talker conditions is not as strong as in Experiment 1a (similar to all other effects in Experiment 1b), we find that performance during test after multi-talker exposure did not fully approach performance after talker-specific exposure. This deviates from BB08.
...
\newpage
# What can we conclude from Experiments 1a and 1b together?

```{r combined experiments 1a and 1b test data}
...
## Meta-analysis (Experiments 1a and 1b)
We combine the data from both experiments and then repeat the same analysis as above. This is the same as updating the posterior of Experiment 1a with the data from Experiment 1b, therefore assuming exchangeability between the two experiments.
...
## Replication analysis (Experiments 1a and 1b)

The code for the replication analysis was developed based on invaluable input by Stephen Martin (see Stan discourse forum at \url{https://discourse.mc-stan.org/t/questions-about-bayesian-replication-test/7450/3}). All errors remain our own. If you develop replication analyses from these scripts, we would appreciate if you cite the article as the source.

For the replication analysis, we use treatment-coded models, rather sliding difference-coded models. We do so because the replication analysis requires comparison of the full models (of Experiment 1b) against models with the effects of interest removed---in order to set those effects to zero (also for Experiment 1b; see Appendix E in the main text). This is most straightforwardly done in the treatment-coded models. For questions 1-2, we use the control condition as reference baseline for the treatment coding. For questions 3, we use the single-talker condition as reference baseline, so that the multi-talker treatment effect in that model corresponds to the difference between multi-talker and single-talker exposure. 
...
my.newpriors.woTS.vs.CNTL = my.newpriors[-which(my.newpriors$class == "b" & my.newpriors$coef == "Cond.treatTS.vs.CNTL"),]
my.newpriors.woMT.vs.CNTL = my.newpriors[-which(my.newpriors$class == "b" & my.newpriors$coef == "Cond.treatMT.vs.CNTL"),]
my.newpriors.woST.vs.CNTL = my.newpriors[-which(my.newpriors$class == "b" & my.newpriors$coef == "Cond.treatST.vs.CNTL"),]
...
## +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
bf.TS.vs.CNTL = bf(b1, b2.woTS.vs.CNTL)
bf.MT.vs.CNTL = bf(b1, b2.woMT.vs.CNTL)
bf.ST.vs.CNTL = bf(b1, b2.woST.vs.CNTL)
...
To test replication of Question 3, we are recoding all treatment contrasts so that the  single-talker condition is the baseline. This allows us to assess replication of the treatment effect of the multi- talker condition, compared to the  single-talker baseline.
...
my.newpriors.woST.vs.MT = my.newpriors.ST.vs.MT[-which(my.newpriors$class == "b" & my.newpriors$coef == "Cond.diffST.vs.MT"),]
...
# Step 6: Calculate all the relevant Bayes Factors
## +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
bf.MT.vs.ST = bf(b1, b2.woST.vs.MT)
bf.MT.vs.ST
```
...
get_replication_draws = function(
  model1, 
  model2, 
  predictor # can be a vector
) {
  model1 %>%
    gather_draws(!!! rlang::syms(predictor)) %>%
    mutate(model = "1a") %>%
    rbind(
      model2 %>%
        gather_draws(!!! rlang::syms(predictor)) %>%
...
effects = c("b_Cond.treatMT.vs.ST", "b_Cond.treatST.vs.CNTL", "b_Cond.treatMT.vs.CNTL", "b_Cond.treatTS.vs.CNTL")
d.m = 
  get_replication_draws(
    model1 = m.exp1a.test.treat, 
    model2 = m.exp1b.test.treat.full, 
    predictor = c("b_Cond.treatTS.vs.CNTL", "b_Cond.treatMT.vs.CNTL", "b_Cond.treatST.vs.CNTL")) %>%
  rbind(
    get_replication_draws(
      model1 = m.exp1a.test.treat.Q3, 
      model2 = m.exp1b.test.treat.full.Q3, 
      predictor = c("b_Cond.treatST.vs.MT")) %>%
      mutate(.variable = "b_Cond.treatMT.vs.ST", .value = -.value)
  ) %>% 
...
p = d.m %>%
  ggplot(aes(x = .value, y = .variable, fill = factor(model))) +
  geom_vline(xintercept = 0, linetype = 2) +
  stat_slabh(color = NA, alpha = .5, scale = .8) +
  ...
  scale_fill_viridis(
    "",
    breaks = c("1a", "1b"),
    labels = c("Prior\n(based on Experiment 1a)", "Posterior\n(after updating w/ data from Experiment 1b)"),
    discrete = T) + 
  coord_cartesian(
    xlim = c(-.3, 1.3),
    ylim = c(.99, 4.8),
...
ggsave(p, filename = "./figures/replication_Q123_effects.pdf", width = 8, height = 4.3)
...
p.inset = d.m %>%
  ...
  summarise_at(vars(.value), .funs = list("mean" = mean, "sd" = sd)) %>%
  crossing(x = x) %>%
  mutate(y = dnorm(x, mean, sd)) %>%
  ggplot(aes(x = x, y = y, fill = factor(model))) +
  geom_vline(xintercept = 0, linetype = 2) +
  geom_area(alpha = .5, position = "identity") +
  geom_segment(
    data = . %>% 
      filter(x == min(abs(x))),
    y = 0, aes(x = x, xend = x, yend = y, color = factor(model)), size = 1, alpha = 1) +
  geom_text(
    data = . %>%
      mutate(label = paste("BF =", case_when(
          .variable == "b_Cond.treatTS.vs.CNTL" ~ round(bf.TS.vs.CNTL$bf, 1),
          .variable == "b_Cond.treatMT.vs.CNTL" ~ round(bf.MT.vs.CNTL$bf, 1),
          .variable == "b_Cond.treatST.vs.CNTL" ~ round(bf.ST.vs.CNTL$bf, 1),
          .variable == "b_Cond.treatMT.vs.ST" ~ round(bf.MT.vs.ST$bf, 1)))) %>%
      group_by(.variable) %>%
      summarise(
        x = min(x) + (max(x) - min(x)) / 45,
        y = max(y) * .88,
        label = first(label)
      ),
    ...
    size = 2.75,
    inherit.aes = F) +
  scale_x_continuous(NULL, breaks = c(min(x), 0), labels = scientific_10) +
  scale_y_continuous("", label = scientific_10) +
  scale_color_viridis(
    "",
    breaks = c("1a", "1b"),
    labels = c("Prior\n(based on Experiment 1a)", "Posterior\n(after updating w/ Experiment 1b)"),
    discrete = T,
    guide = F) + 
  scale_fill_viridis(
    "",
  ...
  facet_wrap(~ factor(.variable, levels = rev(effects)),
             nrow = 4,
             scales = "free_y") +
  theme_bw() +
  theme(
...
l = get_legend(p)
plot_grid(
  plot_grid(p + theme(legend.position = "none"), p.inset, axis = "tb", align = "h", rel_widths = c(.8, .2), rel_heights = c(1, .8)),
  l, nrow = 2, rel_heights = c(.9, .1), axis = "lr")
ggsave(filename = "./figures/replication_Q123_effects_wInset.pdf", width = 10, height = 4.3)

# ggdraw(
#   p + 
#   draw_plot(p.inset, 1.5, .5, .4, 4.5))
```
...
\newpage
# Consequences of by-talker variability (Experiments 1a and 1b)
...
### Summary of findings 
It is clear that the specific effects differ between test talkers. For the present data, these differences are comparatively small: we find the same ordering of relative effect sizes across all test talkers, and the posterior probability for a positive answer to Questions 1-3 is always larger than 50%. The similarity of the effect sizes is perhaps not surprising, since (a) all of the six test talkers we considered in our pilot study were chosen to be of medium proficiency (between 71% to 86% percentage transcription accuracy according to ALLSTAAR database), and (b) of those six, we chose the four test talkers that were not close to ceiling after talker-specific adaptation. In short, our test talkers are representive only of a comparatively homogenous subset of the overall population of Mandarin-accented talkers.
...
### Experiment 1a
We first quantify how the overall result of exposure differs depending on the test talker. This can be visualized by plotting the predicted transcription accuracy during test for each of the four exposure conditions depending on the test talker. In these predictions, we hold the offset term that corrects for individual differences constant  (at 0), so that the plot shows the predicted differences between conditions after indidivual differences have been removed (and set to 0 for all participants). This is not changing the model, which has already been fit with the offset term. We are merely removing the effect of individual differences in visualizing the prediction. The dotplot shows the actual transcription accuracy of individual participants. The pointrange shows the mean and 95% highest posterior density interval of the posterior distribution of the model-predicted transcription accuracy.
...
plot_3effects(m.exp1a.test.diff.wTestTalker, "Experiment 1a by test talker", 
              by = "TestTalker", fill = colors.model[2], alpha = 1, addBFs = T) 
ggsave(filename = "./figures/exp1a_Q123_effects_byTestTalker.pdf", width = 8, height = 3.5)
...
# Write to file
{
  sink(file = "./tables/table_hypotheses_Exp1a_byTestTalker.tex")
  print(table_3effects(m.exp1a.test.diff.wTestTalker, scope = "coef", group = "TestTalkerID", format = "latex"))
  sink()
}
```
...
plot_3effects(m.exp1b.test.diff.wTestTalker, "Experiment 1b by test talker", 
              by = "TestTalker", fill = colors.model[2], alpha = 1, addBFs = T)
ggsave(filename = "./figures/exp1b_Q123_effects_byTestTalker.pdf", width = 8, height = 3.5)
...
# Write to file
{
  sink(file = "./tables/table_hypotheses_Exp1b_byTestTalker.tex")
  print(table_3effects(m.exp1b.test.diff.wTestTalker, scope = "coef", group = "TestTalkerID", format = "latex"))
  sink()
}
```
...
my.priors.wExposureTalker.diff = c(
  # Prior on intercept was necessary for convergence.
  prior(student_t(10,2.10,2.5), class = "Intercept"),
  # Following Gelman et al (2008) and later revisions on wiki.
  prior(student_t(3,0,2.5), class = b),
  # I used a smaller scale to aid convergence, 5 is a common choice for logit models
  prior(cauchy(0,2.5), class = sd), 
...
m.exp1ab.test.diff.wExposure_crossed_with_TestTalker <- brm(
  IsCorrect ~ 
    Cond.diff + offset(individual_training_performance.offset) +
    (1 + Cond.diff | Sentence) + (1 | WorkerID) +
    (1 + Cond.diff | TestTalkerID) + 
  ...
  control = list(adapt_delta = 0.99999, max_treedepth = 15),
  prior = my.priors.wExposureTalker.diff, 
  file = "./models/exp1ab.Test.diff.wExposure_crossed_with_TestTalker")
```
...
### Experiments 1a and 1b together

This is based on the same model but combining the data from both experiments, just like the meta-analysis for the main analysis.

```{r, fig.height=3.5, fig.width=15}
plot_predicted_outcome(
  data = d.exp1ab.test,
  model = m.exp1ab.test.diff.wExposure_crossed_with_TestTalker,
  groups = c("TestTalkerID", "TrainingUnderTestTalkerID"),
  n.draws = 2000,
  experiment = "1ab",
  nrow = 2)
...
```{r, fig.height=20, fig.width=20}
table_3effects(m.exp1ab.test.diff.wExposure_crossed_with_TestTalker,
               scope = "coef", group = "TestTalkerID")
table_3effects(m.exp1ab.test.diff.wExposure_crossed_with_TestTalker,
               scope = "ranef", group = "TestTalkerID")
```
...
The present study serves as a reminder that the uncertainty researchers should maintain about the generalizability of experimental results is often much larger than traditional analyses suggest. It is now broadly recognized that subjects constitute a source of random variability that should be taken into account during statistical analysis if one wants to adequately balance the need for conservativity and the goal to generalize from the experimental sample to populations. Yet, other sources of random variability have largely remained ignored, despite the fact that they can limit generalizability just as much. In particular, it is easy to underestimate the cumulative effect of additional sources of variability (Yarkoni, 2019)---such as the particular talkers chosen in an experiment on speech perception. This tendency to 'remember the result, but forget about the uncertainty associated with it' is perhaps further exaggerated by a habit to think about results in terms of the absence or presence of significant effects (cf. the ''significance filter', Vasishth et al., 2018). Bayesian analyses like the ones we present provide a useful tool to more adequately capture the degree of uncertainty that we should have about the existence of cross-talker generalization, but they do not solve the problem of inflated certainty or generalizability if sources of random variability cannot be assessed due to measurement or design characteristics.

One innovative aspect of the current replications, namely the use of multiple exposure and test talkers, lets us assess the generalizability of our results beyond the specific exposure and test talkers employed in Experiments 1a and 1b. We use the Bayesian mixed-effects logistic regressions with random effects by exposure and test talkers to quantify how much our certainty about the effect of exposure increases if our analyses acknowledge exposure and/or test talkers as sources of random variability. Compared to the main analysis, the strength of support in the analysis by test talker decreased for all effects. This is not only the case at the level of by-talker effect (see Figures in Section 7.1 and 7.2), but also with regard to the overall effects of exposure after marginalizing over the uncertainty about the variability in effects across talkers. As we show in this section, the extent of the increase in the uncertainty that results from the inclusion of random effects by test talkers is worth emphasizing: for all effects in both experiments, the standard deviation of the posterior estimate (i.e., the standard error of their mean) increase by a factor of 3-4 when random effects by test talker are included in the analysis. 
...
# Write to file
{
  sink(file = "./tables/table_hypotheses_Exp1a_withTestTalker.tex")
  print(table_3effects(m.exp1a.test.diff.wTestTalker, scope = "standard", format = "latex"))
  sink()
}

{
  sink(file = "./tables/table_hypotheses_Exp1b_withTestTalker.tex")
  print(table_3effects(m.exp1b.test.diff.wTestTalker, scope = "standard", format = "latex"))
  sink()
}
```
...
# Write to file
{
  sink(file = "./tables/table_hypotheses_Exp1a_withExposureTestTalker.tex")
  print(table_3effects(m.exp1a.test.diff.wExposure_crossed_with_TestTalker, scope = "standard", format = "latex"))
  sink()
}

{
  sink(file = "./tables/table_hypotheses_Exp1b_withExposureTestTalker.tex")
  print(table_3effects(m.exp1b.test.diff.wExposure_crossed_with_TestTalker, scope = "standard", format = "latex"))
  sink()
}
```
...
Once we consider the full uncertainty about test talkers, the only effect for which we have strong evidence to generalize to unseen talkers (BF > 20) is the benefit of talker-specific exposure in Experiment 1a. This should not be surprising. The analysis with random effects by test talker assesses whether our findings are likely to generalize to the larger population of test talkers, rather than `just' across items and participants (for discussion, see Yarkoni, 2019). And, four test talkers are simply not enough to assess whether an effect is likely to hold across the population of talkers, for the same reason that four participants are not enough to assess whether an effect holds across participants. 

This is not *necessarily* a problem. First, if a field (for example, psychophysics) has thoroughly determined that this assumption of cross-participant or cross-talker generalization is likely to hold (for a set of behavioral variables under investigation), making such an assumption in any given experiment constitutes one of many simplifying assumptions made for the sake of being able to draw *any* conclusions. %However, this has not yet been established for speech perception (and many other fields involving higher-level cognition). %For fields such as speech perception, and other domains of the psychological sciences that investigate social or socially-conditioned inferences, however, using stimuli from multiple members of the population (e.g., multiple talkers) can be important if the goal is to adequately capture this source of variability. 

Second, many theoretical questions in the cognitive sciences are tested by assessing the *existence* of certain effects, rather than whether they are always detectable. Even a mechanism that is always operating and central to perception is *not* expected to yield detectable consequences under all circumstances. For the research question at hand, for example, we do not expect similarity-based generalization to always yield successful generalization. In fact, as we have outlined above, this hypothesis predicts that generalization will only yield detectable evidence of cross-talker generalization when the talkers are sufficiently similar. For the present purpose, the critical question is not whether we would find cross-talker generalization after exposure to any single-talker with the same accent, but rather whether we find such generalization when predicted. 
...
\newpage
# Power analysis (Experiment 1a vs. BB08)

For convenience, we provide the result of the power simulations as a data file as part of this git repository. The code in the R markdown document is set up to automatically rerun the power analyses from scratch if that data file is deleted (powersims.RData). 

In order to estimate the power for Questions 1-3 (and talker-specific adaptation, for the sake of comparison), each simulation fit both a model with sliding difference coding for exposure condition, and a model with treatment-coding for condition (see code in R markdown). This means that the three simulations presented below required at least 4 * 1000 * 2 = 8000 GLMM fits. Additional model fits were required whenever a model did not converge, in which case we followed common practice and refit the model (on the same data) with reduced random effect structure. Specifically, we first removed the by-item slope for exposure condition. If that model still did not converge, we removed the by-subject intercepts (100\% of these models converged, so that no further reduction needed to be considered).

On a 2013 PowerMac, ten simulated data sets and analyses for the BB08 data took about 2-3 minutes to complete if run on 12 cores. All three simulations together took about 3 days to complete.

```{r power analysis-functions}
make_cov = function(SD) {
  matrix(
    c(SD^2, rep(0, 15)), 
    nrow = 4,
    dimnames = list(
...
get_full = function(d) {
  with(d, 
       rbind(
         round(prop.table(table(Final.treat == "full")), 2) * 100,
         round(prop.table(table(Final.treat == "full")), 2) * 100,
         round(prop.table(table(Final.treat == "full")), 2) * 100,
         round(prop.table(table(Final.diff == "full")), 2) * 100,
         round(prop.table(table(Final.diff == "full")), 2) * 100
       )[,2])
}

get_significant_and_converged = function(d) {
  with(d, 
       rbind(
...
get_significance_and_final_model = function(d) {
  paste0(
    get_significant_and_converged(d),
    "% (",
    get_full(d),
    "%)"
  )
...
if (!file.exists("./models/powersims.RData")) {
  ...
  subj_SD = .65
  subj.ranef.covar = make_cov(subj_SD)
  ...
  item.sd = item.sd * 2
  item.ranef.covar = MBESS::cor2cov(cor.mat = item.cor, sd = item.sd)
  subject.ranef.covar = make_cov(subj_SD * 2)
  ...
  num.sims = 1000
  n.workers = min(12, parallel::detectCores())
  ...
  save(d.sim.small, file = "./models/powersims-BB08.RData")
  ...
  d.sim = 
    ...
    as_tibble() %>% 
    mutate_all(.funs = unlist)
  save(d.sim, file = "./models/powersims-Exp1a.RData")
  ...
  d.sim.half = 
    ...
    as_tibble() %>% 
    mutate_all(.funs = unlist)
  save(d.sim.half, file = "./models/powersims-Exp1a-half.RData")
  ...
  d.sim.2var = 
    ...
    as_tibble() %>% 
    mutate_all(.funs = unlist)
  save(d.sim.2var, file = "./models/powersims-Exp1a-2var.RData")
...
```{r power analysis-plot}
# Power = proportion of significant effects in expected direction (for which model also converged)
kable(
  matrix(cbind(
    get_significance_and_final_model(d.sim.small),
    get_significance_and_final_model(d.sim),
    get_significance_and_final_model(d.sim.2var),
    get_significance_and_final_model(d.sim.half)
  ), nrow = 5, ncol = 4, 
  dimnames = list(
    c("Adaptation: TS vs. CNTL", "Question 1: MT vs. CNTL", "Question 2: ST vs. CNTL", "Question 3: MT vs. ST", "TS vs. MT"),
    col.names = c("BB08", "Exp 1", "Exp 1 (2 * sigma)", "Exp 1 (.5 beta)")
  )),
  caption = "Percentage of simulations that converged and found the correct (ground truth) effect. Parentheses show percentage of models that converged (regardless of whether the correctly detected the effect. In practice non-converged models would be refit with reduced random effect structure, until a random effect structure is found that yields convergence. Note that the number of converged models is identical for all effects from the same model: talker-specific adaptation and Questions 1-2 are based on the treatment-coded GLMM, the other effects on the sliding difference-coded GLMM."
)


# Wrap sink into single expression to make it work, since knitr already uses sink()
# see https://groups.google.com/forum/#!topic/knitr/TCz9vNLlslY
{
  sink(file = "./tables/table_power_sim.tex")
  print(kable(
  matrix(cbind(
    get_significance_and_final_model(d.sim.small),
    get_significance_and_final_model(d.sim),
...
n.models = 
  rbind(d.sim, d.sim.small, d.sim.half, d.sim.2var) %>%
  mutate_at(vars(Final.treat, Final.diff),
            .funs = function(x) 
              as.numeric(
                factor(x, 
                       levels = c("full", "no by-item slope", "no by-subject")))) %>%
  summarise(n = sum(Final.treat + Final.diff))
```

Total number of models that needed to be fit: `r n.models`.
...
\newpage
# Similarity-based analysis (Experiments 1a and 1b)
```{r}
...
Below we describe the two types of similarity measures, how they were obtained, and whether they predict talker-to-talker generalization in the single-talker condition. To anticipate the results, neither of the two measures is predictive of transcription accuracy during test in the single-talker condition. For our `objective similarity' measure, there is clear evidence that our approach fails to approximate the true objective similarity of the talkers' phonetics in any relevant way, making this test uninformative. For the subjective similarity ratings, the problem seems to be a different one. Our findings suggest that participants are able to judge the perceived similarity of talkers, and these judgments suggests that some of our exposure and test talkers are perceived as substantially more similar than others. However, we find that these subjective similarity ratings seem to primarily rely on basic suprasegmental speech properties, such as the talkers' fundamental frequency (F0). Since these properties are not *necessarily* predictive of how talkers realize segments and words, it is perhaps not surprising that this measure turns out not to reliably predict how much exposure to one talker facilitates accurate perception of another talker.

We nevertheless report our approach and findings here, primarily for two reasons. First, good reporting standard dictates that we report all tests we conducted. Second, we hope that the approaches explored here---flawed as they are---can be informative for other reseachers interested in similar questions.

## Subjective similarity ratings

We investigated whether perceived talker-to-talker similarity explains differences in cross-talker generalization. We obtained sentence-by-sentence similarity ratings for all exposure and test talker pairs from Exp 1 & 2 from an independent group of 48 participants over Mechanical Turk (for a similar task, see Strand, 2000). <!--Johnson 2006 "Resonance in an exemplar-based lexicon" summarizes strand [search for "similarity"] -->

In each trial, listeners heard the same sentence twice, produced by two different talkers (different talker pairs) or by the same talker (same talker pairs), and were instructed to rate their similarity on a scale of 1-7 (7 being very similar). Speech stimuli were identical to those used in Exp 1 & 2 and were organized as follows. Each sentence set (each containing 16 sentences) used for Exp 1 & 2 were split into two halves, resulting in a total of four subsets each containing eight sentences. Experimental lists were constructed such that within each list, all combinations of different talker pairs read eight sentences in one of the four subsets (e.g., Set 1, Half A) and all same talker pairs read the other half of the same sentence set (Set 2, Half B). Talker order was counterbalanced such that for each of the different talker pairs, in half trials talker 1 appeared first and in other half trials talker 2 appeared first. One pseudorandom presentation order was created for each experimental list and presented in two reserved presentation orders. There were a total of 16 lists: 4 subsets X 2 talker order X 2 reversed presentation order. 

Of note, there was no specific instruction to listeners as to what information they should use to judgment similarity. After the experiment, all participants completed a questionnaire. We asked participants to rank 9 different criteria in terms of which they used the most and the least to perform the similiarity judgment task. 

The nine different criteria presented to participants were as follows:

  - The talkers' voice quality (e.g. gruff, happy)
  - How the talkers pronounced words
  - The pitch of the talkers' voices
  - The general rhythm and contour the talkers used (prosody)
  - Whether the sentences contained stutters or other disfluencies
  - Intelligibility of the talkers
  - How manly the talkers sounded
  - The speaking rate of the talkers
  - Other (specify below)

### Methods

For each participant, we first computed the mean similarity rating for each unique exposure-test talker pairs (20 different talker pairs + 4 same talker pairs) by averaging across sentences. These by-participant ratings for talker pairs were entered into further analyses reported below. 
...
d.for_sim = d.exp1ab.test %>%
  filter(Condition %in% c("Single-talker", "Talker-specific")) %>%
  mutate(
    TalkerPair = paste(
  ...
  group_by(Experiment, Condition, WorkerID, Sentence, TalkerPair, TestTalker, TrainingTestSet) %>%
  summarise(
...
d.sim.subj %>%
  ...
  arrange(Condition, Similarity_order) %>%
...
To analyze whether subjective similarity between the exposure and test talker predicts the accuracy during test, we calculated the mean similarity rating for each unique combination of the 20 different pairs of exposure and test talker, and the two different exposure-test sentence sets. 
...
We note that similarity ratings *are* predictive of performance during test if similarity ratings are *not* averaged for each unique combination of exposure talker, test talker, and exposure-test sentence sets---both for Experiment 1a and 1b: 
...
If similarity ratings---which are available for each unique pair of recordings of the same sentence---are instead entered into the mixed-effects logistic regression as-is---i.e., while allowing to vary between sentences---we find very strong support for the hypothesis that subjective similarity predicts talker-to-talker generalization (BFs > 150). However, this might simply reflect that a) raters gave lower similarity scores to pairs of sentences when one sentence differed from the other in terms of overt disfluences or speech errors, and b) the presence of overt disfluencies or speech errors also made it harder to correctly transcribe a sentence. In that case, sentence-level similarity would correlate with sentence-level transcription accuracy for reasons independet of successful talker-to-talker transfer. Given that we did not find the talker-level similarity ratings to be consistently predictive of performance during test, we believe this to be the most likely explanation for the very strong correlation between sentence-level similarity ratings and performance during test.
...
Alternatively, one could measure the similarity of talker pairs based on the phonetic properties of their production. However, in a transcription paradigm that employs speech naturally produced by foreign-accented talkers, it is unclear what phonetic contrasts or acoustic properties can facilitate perception of foreign-accented speech and therefore be examined in relation to talker similarity. As an attempt, we analyzed the vowel productions of all talkers used in Exp 1 & 2 and used the similarity of vowel production as a first approximation for phonetic similarity in general. 
...
That said, the approach taken here is limited in many ways. Although vowels generally are important in determining speech intelligibility (Kewley-Port et al., 2007), they are not necessarily the perceptually most advantageous sounds in improving the intelligibility of Mandarin-accented speech. In addition, our experimental lists were not designed to be phonetically balanced. As a result, the number of tokens across vowel categories may be insufficient; the production of specific vowel tokens may be influenced by many factors that were not controlled for, such as phonetic contexts, word frequency, etc. 


### Methods

We analyzed ten vowels /i, ɪ, e, æ, ɛ, ʌ, ɑ, o, ɔ, u/ that were relatively well-present in our stimuli (having at least three tokens per talker). Formants (F1, F2) measurements were measured in Praat at three points (20%,50%,80%) for each target vowel based on default Burg formant tracking analyses (5 formants in 5500 Hz; 25 ms window) and verified by visual examination of wide band spectrograms. Midpoints (50%) were used in the analyses reported here. 

To determine the similarity between talkers within each talker pair, we ask how well an ideal observer recognizes the vowels (based on Lobanov-normalized F1 and F2) produced by the second talker if it is trained with vowel tokens from the first talker. Using the vowel productions from each talker, ideal observers were trained to estimate vowel-specific cue distributions as bivariate Gaussian distributions. Then these ideal observers were applied to the vowels from different test talkers in a vowel categorization task (10 alternative forced choice). If there were sufficient training tokens, then the test performance would be best when the training talker and the test talker were the same; the more similar the two talkers' vowel productions are, the higher test accuracy would be for a given test talker as a function of the training talker. 


### Summary of ideal observer-derived accuracies

The following plots show the predicted accuracy of an ideal observer applied to the vowels of the test talker, based on training on the vowels of the exposure talker. The six talkers to the right of the dividing line correspond to the talker-specific condition. To the left of the dividing line are predicted accuracies for all 20 combinations of exposure and test talker in the single-talker condition. What is striking is that accuracy is *not* always highest in the talker-specific condition, despite the fact that in this case the exposure stimuli that the model is trained on come from the same talker as the test stimuli that the model is tested on. This provides strong evidence that the models are overfit to the training data, presumably because we simply do not have sufficiently many instances of the different vowels in the training data. In line with this hypothesis, the relative ordering of the different single-talker models differs quite a bit depending on which set of sentences was used for training and which set was used for test (see second plot).
...
\newpage
# Do results depend on when participant completed experiment? (Experiments 1a and 1b)
...
d.exp1ab.training %>% 
  ...
  ggplot(aes(x = days + 1, y= Correct, color = Condition)) + 
...
We visualize the transcription accuracy during test first by exposure condition, and then show the same split by test talker. The figures use the same averaging and bootstrapped confidence intervals as used for Experiments 1a and 1b. 

```{r visualization of pilot data} 
myGplot.defaults(type = "paper")
p.test.proportion %+% 
  (d.pilot %>%
  group_by(WorkerID, Sentence) %>%
...
p = p.test.proportion %+% 
  ...
  distinct(.keep_all = TRUE) %>%
  filter(PartOfExp == "test") %>% 
  group_by(WorkerID, Condition, CurrentTalkerID) %>%
	dplyr::summarise(PropKeywordsCorrect = mean(PropKeywordsCorrect)))

p +
  geom_text(
    data = d %>%
      group_by(Condition, CurrentTalkerID) %>%
      dplyr::summarise(PropKeywordsCorrect = mean(PropKeywordsCorrect)) %>%
...
Similarly, we followed the same Baysian mixed-effects logistic regression analysis as for Experiments 1a and 1b, with one exception: since the pilot experiment only has two conditions (control and talker-specific exposure), we use simple treatment coding for the exposure condition (1 = talker-specific, 0 = control). Like the analyses of Experiments 1a and 1b, the regression contained an offset term to correct for individual differences between participants, as well as the maximal random effect structure (random intercepts by participant and item, as well as by-item random slopes for condition). The offset term was obtained by fitting a separately model to the exposure data of the pilot experiment, following the exact same approach as in Experiments 1a and 1b.
...
m.brm.pilot.train = brm(IsCorrect ~ Cond.treat * sQuintile +
                           (1 + Cond.treat | Sentence) + (1 + sQuintile | WorkerID) + (1 | CurrentTalkerID), 
                         data = d.pilot.training, 
                         family = bernoulli, cores = max.cores, chains = 4, 
                         warmup = 2000, iter = 4000, 
                         prior = my.priors, file = "./models/pilot.Training.treat")
...
d.pilot.test %<>% 
  add_individual_performance() %>% 
  droplevels()
contrasts(d.pilot.training$Cond.treat) = cbind("TS.vs.CNTL" = c(1,0))
contrasts(d.pilot.test$Cond.treat) = cbind("TS.vs.CNTL" = c(1,0))


m.brm.pilot.test.fromIndividualTrainingDifferences <- brm(
  IsCorrect ~ 
    individual_training_performance.offset +
    (1 | Sentence) + (1 | WorkerID),  
  data = d.pilot.test, family = bernoulli, cores = max.cores, chains = 4, 
  warmup = 2000, iter = 4000, 
  file = "./models/pilot.Test.treat.fromIndividualTrainingDifferences")

m.brm.pilot.test <- brm(
  IsCorrect ~ 
    offset(individual_training_performance.offset) +
    1 + Cond.treat + 
    (1 + Cond.treat | Sentence) + (1 | WorkerID),  
  data = d.pilot.test, family = bernoulli, cores = max.cores, chains = 4, 
  warmup = 2000, iter = 4000, 
  file = "./models/pilot.Test.treat")
```

## Summary of exposure analysis
```{r, pilot exposure block analysis - model output, size = "footnotesize"}
summary(m.brm.pilot.train)
```

## Summary of test analysis

Predicting test from individual differences at onset of exposure:
```{r, pilot test block analysis by inidividual differences - model output, size = "footnotesize"}
summary(m.brm.pilot.test.fromIndividualTrainingDifferences)

xhypothesis(hypothesis(m.brm.pilot.test.fromIndividualTrainingDifferences, "individual_training_performance.offset > 0", class = "b"))
```

```{r, pilot test block analysis - model output, size = "footnotesize"}
summary(m.brm.pilot.test)

xhypothesis(hypothesis(m.brm.pilot.test, "Cond.treatTS.vs.CNTL > 0", class = "b"))
```
...
# Additional small-scale replications

## Does increased variability during exposure affect transcription accuracy during test? (Experiment 2)

This small control experiment was conducted in order to assess whether the order of stimuli stimuli during exposure might affect cross-talker generalization (as suggested by some work, e.g., Tzeng et al., 2016). At some point while collecting data for Experiment 1a, we became aware that the order of stimuli during exposure employed in our replication differed from that employed in BB08 (see paper for discussion). At that point, we thus conducted Experiment 2 with the primary goal to assess whether increased variability during exposure facilitates cross-talker generalization during test. Facilitation is predicted by the type of variability-based account advanced in BB08, and has indeed been observed in some studies (Tzeng et al., 2016). Here, we thought to go beyond small changes in the order of stimuli, and instead drastically increase variability during exposure (while holding the test stimuli constant). 

The results of this control experiment should, however, be interpreted with caution given the small number of participants. 

### Participants 

```{r}
d.order %<>%
  # Excluding this test talker since it was only used in the control condition
  filter(TestTalkerID != "CMN_M_035") %>%
  # Excluding all control subjects since are just not enough left
  filter(Condition != "Control") %>%
  droplevels()
```
We recruited a total of 69 additional participants in the single and multi-talker conditions for which we wanted to compare the effect of exposure variability. None of the participants met any of the exclusion criteria. We note that an original presentation of this control experiment at the 2016 CUNY Conference on Sentence Processing mistakenly included (additional) participants that had been tested on stimuli that were also part of exposure. These participants were excluded here.
...
### Materials
The materials were identical to Experiments 1a and 1b, except that we used an additional 64 different sentences, resulting in a total of 96 instead of 32 sentences. Following Experiments 1a and 1b, we divided these sentences in sets of 16, where set 1 and set 2 are the two sets used in Experiments 1a and 1b. As in Experiments 1a and 1b, either set 1 or set 2 was used as test set. All other five sets of 16 sentences were used as exposure stimuli, resulting in the following fully balanced distribution of stimuli across conditions:

```{r}
d.order %>%
  group_by(Condition) %>%
  dplyr::select(TrainingTestSet) %>%
  distinct()
```

We used only three of the four test talkers from Experiments 1a and 1b, the ones for which we had materials available when Experiment 2 was conducted.

```{r}
d.order %>%
  group_by(Condition) %>%
  dplyr::select(TestTalkerID) %>%
  distinct()
```

As in Experiments 1a and 1b, we recruited data for all possible combinations of exposure and test talkers in the single-talker condition. Since we have a total of six talkers to choose from (as in Experiments 1 an and 1b) but only three test talkers for Experiment 2 (rather than four), this resulted in 15 unique combinations:

```{r}
d.order %>%
  filter(Condition %in% "Single-talker") %>%
  group_by(Condition) %>%
  dplyr::select(TrainingTalkerID, TestTalkerID) %>%
  distinct()
```

### Test results

We compare the results to 72 randomly sampled participants from the single and multi-talker conditions of Experiment 1a (which was run around the same time as Experiment 2). We sampled these participants only from participants who had been tested on the same three test talkers as in Experiment 2. An equal number of participants (four) was sampled for each unique combination of condition, test talker, and test set (which of the two sets of 16 sentences was used during test).
...
d.exp1a.test.sub =
  d.exp1a.test %>%
  filter(
    TestTalkerID != "CMN_M_035",
    Condition %in% c("Single-talker", "Multi-talker")) %>%
  droplevels() %>%
  group_by(Condition, TestTalkerID, TrainingTestSet) 

d.exp1a.test.sub %<>%
  filter(WorkerID %in% sample(unique(WorkerID), 4)) %>%
  droplevels() %>% 
  ungroup()
...
p.test.proportion %+% 
  (d.order.test %>%
     dplyr::select(Experiment, WorkerID, Sentence, Condition, PartOfExp, PropKeywordsCorrect) %>%
     rbind(d.exp1a.test.sub %>%
             dplyr::select(Experiment, WorkerID, Sentence, Condition, PartOfExp, PropKeywordsCorrect)) %>%
     group_by(Experiment, WorkerID, Sentence) %>%
     distinct(.keep_all = TRUE) %>%
     group_by(Experiment, WorkerID, Condition) %>%
	dplyr::summarise(PropKeywordsCorrect = mean(PropKeywordsCorrect))) + 
  aes(shape = Experiment) + theme(legend.position = "bottom")
ggsave(filename = "./figures/p.exp2.pdf", device = cairo_pdf, width = 3, height = 4, dpi=300)
...
p = p.test.emplogit %+% (
  d.order.test %>%
    ...
    group_by(WorkerID, Condition) %>%
    dplyr::summarise(PropKeywordsCorrect = emplog(mean(PropKeywordsCorrect), 51)))
ggsave(p, filename = "./figures/p.exp2.emplogit.pdf", device = cairo_pdf, width = 3, height = 4, dpi=300)
```

### Discussion

The results of this comparison do indeed suggest that multi-talker exposure might benefit from increased variability during exposure. No such benefit was observed for single-talker exposure. Since these results are based on a rather small number of participants, they should be interpreted with caution. Taken at face value, these results are problematic for the assumption that variability always facilitates cross-talker generalization.

...
d.sim.subj %>%
  ...
  group_by(TalkerPair, TestTalker, Sentence) %>%
...
run_subjective_similarity_analysis = function(
  ...
  m = brm(
    ...
    data = d %>%
      ...
      # # Delete next three rows to NOT average by exposure-test talker
      # # (instead keeping separate similarity values for each sentence)
      # group_by(TalkerPair) %>%
...
print(hypothesis(m.exp1a.subjective_similarity.ST, "Similarity > 0"))
...
print(hypothesis(m.exp1b.subjective_similarity.ST, "Similarity > 0"))
...
d.sim.obj = 
  ...
  left_join(
    d.similarity.objective %>%
      ...
      group_by(TalkerPair, Sentence, modelType) %>%

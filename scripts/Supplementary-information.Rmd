...
plot_predicted_outcome = function(
  ...
  d = data %>%
    group_by(WorkerID, TestTalkerID, TrainingTalkerID, TrainingUnderTestTalkerID, Cond.diff, !! sym(offset.term), Sentence) %>%
  ...
  m = d %>%
    ...
    group_by(!!! syms(groups), Cond.diff, .draw) %>%
    summarise(.value = mean(.value)) %>%
    mean_hdci()
    
  myGplot.defaults("paper")
  p = m %>%
    ggplot(aes(x = Cond.diff, color = Cond.diff, fill = Cond.diff, y = .value)) +
    # actual subject data
    geom_dotplot(
      binaxis = "y",stackdir = "center", width = .1, binwidth = .01, 
      data = d %>% 
        group_by(WorkerID, Cond.diff, !!! syms(groups)) %>%
  ...
  ggsave(p, filename = paste0("../figures/exp", experiment, "_predicted outcome_by", paste(groups, collapse = "+") ,".pdf"),
         height = 7, width = 7)
...
plot_predicted_outcome(
  ...
  n.draws = 2000,
  experiment = "1a",
  offset.term = "individual_training_performance.linear")
```
...
### Experiment 1a (ADD RESULTS DESCRIPTION)
...
Next, we plot the Bayesian hypotheses tests for Questions 1-3. Note that the results for talker-specific adaptation and Question 1 (comparing the multi-talker condition against the control) do not change. The results for Questions 2 and 3, however, do depend on the mean of the single talker condition, and thus change in the updated model.
...
### Experiment 1b (ADD RESULTS DESCRIPTION)
```{r, fig.height=3.5, fig.width=15}
plot_predicted_outcome(
  data = d.exp1b.test,

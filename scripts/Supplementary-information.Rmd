...
All models were fit with the settings described in the main text. We note that substantially fewer posterior samples would have been sufficient. A larger number of posterior samples is required for the replication test, but separate (numerically-coded) models were fit for that test anyway. The reason for using equally many warmup and posterior samples for the main analysis was to simplify the write-up.
...
Due to space limitations, we do not discuss all results of theoretical relevance in the main text. Beyond talker-specific adaptation and Questions 1-3, the differences between the talker-specific and multi-talker conditions is also of interest. It indicates whethercross-talker generalization was essentially as good as the best possible performance. BB08 found no signficiant difference between their talker-specific and multi-talker condition. Here, we do not replicate that result. 
...
We investigated whether perceived talker-to-talker similarity explains differences in cross-talker generalization. We obtained sentence-by-sentence similarity ratings for all exposure and test talker pairs from Exp 1 & 2 from 48 participants over Mechanical Turk (for a similar task, see Strand, 2000). <!--Johnson 2006 "Resonance in an exemplar-based lexicon" summarizes strand [search for "similarity"] -->

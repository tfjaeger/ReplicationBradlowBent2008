...
plot_3effects = function(model, title, add_plot = NULL, 
                         ...
                         by = c("", "TestTalker", "TestTalkerID:TrainingTalkerID")[1]
) {
...
prepare_talkerIDs = . %>%
  ...
  mutate(
    ...
    TrainingUnderTestTalkerID = case_when(
      Condition2 == "Single talker" ~ TrainingTalkerID,
      T ~ "0"
    ),
...
m.exp1a.test.diff.wExposure_crossed_with_TestTalker <- brm(IsCorrect ~ Cond.diff + offset(individual_training_performance.offset) +
                                   (1 + Cond.diff | Sentence) + (1 | WorkerID) +
                                   (1 + Cond.diff | TestTalkerID) + 
                                   (1 | TrainingUnderTestTalkerID),
                                 data = d.exp1a.test %>%
                                   mutate(TrainingUnderTestTalkerID =
                                            ifelse(Condition2 == "Single talker",
                                                   paste(TrainingUnderTestTalkerID, TestTalkerID, sep = "__"),
                                                   "0")), 
                                 family = bernoulli, cores = max.cores, chains = 4,
                                 warmup = nsamples.warmup.wTalker, iter = nsamples.total.wTalker,
...
table_3effects(m.exp1a.test.diff.wExposureTestTalker, 
...
table_3effects(m.exp1b.test.diff.wExposureTestTalker, 
               scope = "coef", group = "TestTalkerID:TrainingTalkerID")
table_3effects(m.exp1b.test.diff.wExposureTestTalker, 
               scope = "ranef", group = "TestTalkerID:TrainingTalkerID")
```
...
```{r}
table_3effects(m.exp1a.test.diff.wExposureTestTalker, scope = "standard")
table_3effects(m.exp1b.test.diff.wExposureTestTalker, scope = "standard")
```

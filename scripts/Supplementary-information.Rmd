...
# TO DO
 + RECODE EXPOSURE-TEST TALKER COMBINATION
   ++ UPDATE FUNCTIONS TO DEAL WITH NEW CODING.
 + INCLUDE objective SIMILARITY ANALYSIS?
...
my_scale = function(x) (x - mean(x)) / (2 * sd(x))
...
plot_3effects = function(model, title, add_plot = NULL, 
    ...
    { 
      if (by == "ExposureTestTalker") {
        left_join(
          ...
          # Filter out cases that do not occur in the data (e.g., all the Exposure-Test-Talker
          # combinations for anything but the ST conditions).
          # filter(
          #   any(
          #     TestTalker == gsub(ExposureTest)
          #     condition != ExposureTestTalker == "0",
          #       ))
      } else . 
    } %>%
    pivot_wider(
...
Separated by exposure condition, we see some variability in the exact estimate of how much individual differences estimated during exposure affect performance during test, but overall the estimated coefficients show a remarkable degree of agreement across the four (entirely non-overlapping) test data for the four different exposure conditions:
...
```
Going back to the combined test data from all four exposure conditions, the effect of the individual differences predictor can also be seen in the reduction of the variability in the by-participant random intercepts during test in the model that controlled for individual differences in performance (SD = .60):
...
m.exp1a.test.diff.wTestTalker <- brm(IsCorrect ~ Cond.diff + offset(individual_training_performance.offset) + 
...
m.exp1b.test.diff.wTestTalker <- brm(IsCorrect ~ Cond.diff + offset(individual_training_performance.offset) + 
                                   (1 + Cond.diff | Sentence) + (1 | WorkerID) + (1 + Cond.diff | TestTalkerID),  
...
## Do differences between exposure-test talker combinations affect the conclusions one would draw? I AM NOT SURE THIS ACTUALLY MAKES SENSE. WE CAN'T REALLY ESTIMATE SLOPES FOR EXPOSURE-TEST TALKER COMBINATIONS
...
# m.exp1a.test.diff.wExposureTestTalker <- brm(IsCorrect ~ Cond.diff + offset(individual_training_performance.offset) + 
#                                    (1 + Cond.diff | Sentence) + (1 | WorkerID) + 
#                                    (1 + Cond.diff | TestTalkerID / TrainingTalkerID),  
#                                  data = d.exp1a.test, family = bernoulli, cores = max.cores, chains = 4, 
#                                  warmup = nsamples.warmup.wTalker, iter = nsamples.total.wTalker, 
#                                  control = list(adapt_delta = 0.9999, max_treedepth = 15),  
#                                  prior = my.priors.wExposureTestTalker.diff, file = "../models/exp1a.Test.diff.wExposureTestTalker")
...
table_3effects(m.exp1a.test.diff.wExposure_crossed_with_TestTalker, 
               scope = "coef", group = "TestTalkerID:TrainingTalkerID")
table_3effects(m.exp1a.test.diff.wExposure_crossed_with_TestTalker, 
               scope = "ranef", group = "TestTalkerID:TrainingTalkerID")
```

### Experiment 1b
```{r, fig.height=20, fig.width=20}
plot_3effects(m.exp1b.test.diff.wExposure_crossed_with_TestTalker, 
...
my.newpriors.woST.vs.MT = my.newpriors[-2,]
...
To assess whether inter-talker similarity affects generalization from exposure to test talkers, we analyzed the accuracy in the single talker condition during test for all 20 unique combinnations of exposure and test talkers. We did so for two approaches to measuring similarity, both of them at best thought of as first approximations: a) subjective ratings of similarity obtained from an independent set of participants and b) the objective similarity between talkers in terms of their vowel productions. We describe these measures, how they were obtained, and whether they predict talker-to-talker generalization in the single talker condition.
...
The following figure shows the mean similarity ratings and 95% bootstrapped confidence interval for each combination of exposure and test talkers. The six talkers to the right of the dividing line are the self-similarity ratings for the six talkers used during exposure. To the left of the dividing line are similarity ratings for all 30 possible combinations of these six talkers as exposure and test talker (only 20 of which were actually used in Experiments 1a and 1b). 

![image](../figures/similarity across talkers.png)

We do not know what exactly these similarity scores reflect---i.e., which properties of talkers' speech affected participants' perception of the sentence-to-sentence similarity. One worry is that explicit similarity judgments of the type used here are a form of meta-cognitive judgment, and that this type of judgment draws on different features than those relevant to the implicit (subconscious) processes underlying cross-talker generalization during speech perception. Specifically, we were worried that participants would rely on salient supra-segmental speech features, such as rythm and pitch. These feature are {\em not} necessarily reliable indicators of how similar two talkers are in their realization of sound categories. 
...
![image](../figures/multi-dimensional scaling.png)

This result has to be interpreted with caution, of course, since we only have six different talkers here. It does, however, provide {\it a priori} reason to expect the subjective similarity ratings to {\em not} be a good predictor of successful cross-talker generalization. Even if listeners do use supra-segmental features to generalize experience from exposure to test, this generalization will only facilitate listeners' performance during test to the extent that the exposure and test talker are indeed similar in how they realize sound categories and words.
...
run_subjective_similarity_analysis = function(
  ...
  m = brm(
    IsCorrect ~ 
      offset(individual_training_performance.offset) + Similarity.Ordered + 
      (1 | Sentence) + (1 | WorkerID),  
    data = d %>%
      filter(
      ...
      left_join(d.sim) %>%
      mutate(Similarity.Ordered = my_scale(Similarity.Ordered)) %>%
      dplyr::select(IsCorrect, 
                    WorkerID, Sentence, 
                    Cond.diff, individual_training_performance.offset, Similarity.Ordered), 
    family = bernoulli, cores = max.cores, chains = 4, 
    warmup = nsamples.warmup, iter = nsamples.total, 
    prior = my.priors, file = paste0("../models/", filename))
  
  print(hypothesis(m, "Similarity.Ordered > 0"))
  
  return(m)
}

# Single talkers
m.exp1ab.subjective_similarity.ST = run_subjective_similarity_analysis(
  d.exp1ab.test, 
  filename = "exp1ab.subjective_similarity.ST")
...
The following plots show the predicted accuracy of an ideal observer applied to the vowel's of the test talker, based on training on the vowels of the exposure talker.  

```{r}
load("../data/talker-similarity.RData")

# Prepare IO data frame
d.IO.full %<>%
  mutate(
    learningType = plyr::mapvalues(
      learningType,
...
# Merge information with test data
d = d.exp1ab.test %>%
  filter(
    Condition2 %in% c("Single talker", "Talker-specific")) %>%
  mutate(
    TalkerPair = paste(
      gsub("^.*([0-9]{3})$", "\\1", TrainingTalkerID), 
      gsub("^.*([0-9]{3})$", "\\1", TestTalkerID), 
  ...
  group_by(Experiment, Condition2, WorkerID, Sentence, TalkerPair, TestTalker) %>%
  summarise(
  ...
  left_join(
    d.IO.full %>%
      ...
      group_by(TalkerPair, Sentence, learningType) %>%
...
d %>%
  ...
  arrange(learningType, accuracy_order) %>%
...
last_plot() +
  ...
  facet_wrap(~ TestTalker, scales = "free_x") 
  ...
  m = brm(
    ...
    data = d %>%
      filter(
        Experiment %in% experiment,
        Condition2 == condition) %>%
      mutate(accuracy_logit = qlogis(accuracy)), 
    family = binomial, cores = max.cores, chains = 4, 
    warmup = nsamples.warmup, iter = nsamples.total, 
    prior = my.priors, file = paste0("../models/", filename))
  
  print(hypothesis(m, "accuracy_logit > 0"))
  
  return(m)
}

# Single talkers
m.exp1ab.objective_similarity.ST = run_objective_similarity_analysis(
  d, 
  filename = "exp1ab.objective_similarity.ST")
...
```
For Experiment 1a, there is no support for the the hypothesis that the ideal observer predicted accuracy on the test talker's vowels predicts the actual accuracy of human listeners during test:

```{r}
summary(m.exp1a.objective_similarity.ST)
```

The same holds for Experiment 1b:

```{r}
summary(m.exp1b.objective_similarity.ST)
```

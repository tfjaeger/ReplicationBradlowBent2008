...
For convenience, we provide the result of the power simulations as a data file as part of this git repository. The code in the R markdown document is set up to automatically rerun the power analyses from scratch if that data file is deleted (powersims.RData). On a 2013 PowerMac, ten simulated data sets and analyses for the BB08 data took about 2-3 minutes to complete if run on 10 cores.
...
kable(
  ...
  caption = "Percentage of simulations that found the correct (ground truth) effect. Numbers in parentheses show power when only converged models are counted. Numbers outside of parentheses show power when both converged and non-converged models are counted. In practice non-converged models would be refit with reduced random effect structure, until a random effect structure is found that yields convergence."
)
